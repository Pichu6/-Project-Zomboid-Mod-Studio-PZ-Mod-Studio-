...
pub fn ensure_master_patch_exists_on_disk(user_zomboid_dir: &str) {
    if user_zomboid_dir.is_empty() {
        return;
    }
    let patch_dir = Path::new(user_zomboid_dir).join("mods").join("Z_PZModStudio_MergedPatch");
    ...
    let poster_path = patch_dir.join("poster.png");
    if !poster_path.exists() {
        let _ = fs::write(&poster_path, crate::patch_generator::DUMMY_PNG_BYTES);
    }
    let icon_path = patch_dir.join("icon.png");
    if !icon_path.exists() {
        let _ = fs::write(&icon_path, crate::patch_generator::DUMMY_PNG_BYTES);
    }

    let polyfill_dir = patch_dir.join("media").join("lua").join("shared");
...
pub fn scan_all_installed_mods(paths: &StudioPaths) -> Vec<ModManifest> {
    ensure_master_patch_exists_on_disk(&paths.user_zomboid_dir);
